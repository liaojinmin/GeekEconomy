...
object GPoints {
    ...
    @KetherParser(value = ["GPoints"], shared = true)
    fun parser() = scriptParser {
        it.switch {
            case("give") {
                ...
                actionNow {
                    newFrame(a).run<String>().thenApply { e ->
                        val ac = e.toInt()
                        if (ac > 0) {
                            player().getDataCache()?.givePlayerPoints(ac, true)
                        }
                    }
                }
            }
            case("take") {
                ...
                actionNow {
                    newFrame(a).run<String>().thenApply { e ->
                        val ac = e.toInt()
                        if (ac > 0) {
                            player().getDataCache()?.takePlayerPoints(ac, true)
                        }
                    }
                }
            }
            case("get") {
                actionNow {
                    player().getDataCache()?.getPlayerPoints() ?: "0"
                }
            }
            case("has") {
                val a = it.next(ArgTypes.ACTION)
    ...
    class Has(private val key: ParsedAction<*>): KetherSub<Boolean>() {
        override fun run(frame: ScriptFrame): CompletableFuture<Boolean> {
            return frame.newFrame(key).run<Any>().thenApply {
                ...
                (getPlayer(frame).getDataCache()?.getPlayerPoints() ?: 0) >= ac
            }
        }
    }
}