 ...
 */
@CommandHeader("points", aliases = ["点券"])
    ...
    val look = subCommand {
        execute<Player> { sender, _, _ ->
            sender.getDataCache()?.let {
                sender.sendMessage("§7你的点券: ${it.getPlayerPoints()}")
            }
        }
        dynamic("玩家") {
                ...
                execute<ProxyCommandSender> { sender, context, _ ->
                    ...
                    if (money != null) {
                        getDataCache(target.uniqueId)?.let {
                            if (it.givePlayerPoints(money)) {
                                sender.sendMessage("""
                                    §a|
                                    §a|§7成功给予玩家: §f$money §7点券
                                    §a|§7可用余额: §f${it.getPlayerPoints()}
                                    §a|
                                    """.trimIndent())
                            } else {
        ...
        dynamic("玩家") {
            ...
            dynamic("数量") {
                execute<ProxyCommandSender> { sender, context, _ ->
                    ...
                    if (money != null) {
                        getDataCache(target.uniqueId)?.let {
                            if (it.takePlayerPoints(money)) {
                                sender.sendMessage("""
                                    §a|
                                    §a|§7成功扣除玩家: §f$money §7点券
                                    §a|§7可用余额: §f${it.getPlayerPoints()}
                                    §a|
                                    """.trimIndent())
                            } else {
