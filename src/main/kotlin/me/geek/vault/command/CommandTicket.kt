 ...
 */
@CommandHeader("ticket", aliases = ["月券"])
object CommandTicket {

    @CommandBody
    val look = subCommand {
        execute<Player> { sender, _, _ ->
            sender.getDataCache()?.let {
                sender.sendMessage("§7你的月券: ${it.getPlayerTicket()}")
            }
        }
        dynamic("玩家") {
                ...
                execute<ProxyCommandSender> { sender, context, _ ->
                    ...
                    if (money != null) {
                        getDataCache(target.uniqueId)?.let {
                            if (it.givePlayerTicket(money)) {
                                sender.sendMessage("""
                                    §a|
                                    §a|§7成功给予玩家: §f$money §7月券
                                    §a|§7可用余额: §f${it.getPlayerTicket()}
                                    §a|
                                    """.trimIndent())
                            } else {
        ...
        dynamic("玩家") {
            ...
            dynamic("数量") {
                execute<ProxyCommandSender> { sender, context, _ ->
                    ...
                    if (money != null) {
                        getDataCache(target.uniqueId)?.let {
                            if (it.takePlayerTicket(money)) {
                                sender.sendMessage("""
                                    §a|
                                    §a|§7成功扣除玩家: §f$money §7月券
                                    §a|§7可用余额: §f${it.getPlayerTicket()}
                                    §a|
                                    """.trimIndent())
                            } else {
